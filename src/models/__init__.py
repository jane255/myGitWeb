...
from src.utils import log, get_now_timestamp
...
class Model(object):
    ...
    def db_path(cls):
        ...
        path = f'{project_dir}/data/{classname}.txt'
        return path
    ...
    def find_by(cls, **kwargs):
        ...
        """
        # log('find_by kwargs, ', kwargs, type(kwargs))
        k, v = '', ''
        for key, value in kwargs.items():
            k, v = key, value
        all = cls.all()
    ...
    def save(self):
        ...
        # 如果没有 id，说明是新添加的元素
        if self.id is None:
            # 设置 create_time、update_time
            self.create_time = get_now_timestamp()
            self.update_time = get_now_timestamp()
            # 设置 self.id
            # 先看看是否是空 list
            if len(models) == 0:
